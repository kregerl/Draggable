...
//			int l = 91;
			...
			if (shouldRenderSelector) {
				screen.blit(x - 1 + playerentity.inventory.currentItem * 20, /* this.scaledHeight */y - 1, 0, 22, 24,
						22);
			}
			if (!itemstack.isEmpty()) {
				if (handside == HandSide.LEFT) {
					screen.blit(x - 29, /* this.scaledHeight */y - 1, 24, 22, 29, 24);
				} else {
					screen.blit(x + 91, /* this.scaledHeight */y - 1, 53, 22, 29, 24);
				}
			}
			...
			if (!itemstack.isEmpty()) {
				int i2 = /* this.scaledHeight */y + 3;
				if (handside == HandSide.LEFT) {
					renderHotbarItem(x - 26, i2, screenWidth, screenHeight, partialTicks, playerentity, itemstack);
				} else {
					renderHotbarItem(x - 26, i2, screenWidth, screenHeight, partialTicks, playerentity, itemstack);
				}
			}
...
//			System.out.println(j);
	...
	private static void renderHotbarItem(int x, int y, int width, int height, float partialTicks, PlayerEntity player,
			ItemStack stack) {
		if (!stack.isEmpty()) {
